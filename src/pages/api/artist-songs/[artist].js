...
export default function handler(req, res) {
  ...
  try {
    ...
    // Add percentage to each song
    const songsWithPercentage = sortedSongs.map((song) => ({
      ...song,
      percentage: totalArtistPlays > 0 ? (song.totalPlays / totalArtistPlays) * 100 : 0,
    }));
    ...
    console.error("Error processing artist songs:", error);
    res.status(500).json({
