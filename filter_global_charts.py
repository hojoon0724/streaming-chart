...
def filter_global_charts():
        ...
        for row in reader:
            if row['country'] == 'global':
                ...
                try:
                    ...
                    genres = eval(row['artist_genres']) if row['artist_genres'].startswith('[') else [row['artist_genres']]
    ...
    with open(output_csv, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['date', 'position', 'streams', 'track_id', 'artists', 'genres', 'duration_ms', 'explicit', 'track_name']
